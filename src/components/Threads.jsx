...
export default function Threads({
  ...
  useEffect(() => {
    ...
    const maxTextureSize = 1920;
    function resize() {
      const { clientWidth, clientHeight } = container;
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      const largestSide = Math.max(clientWidth, clientHeight) * dpr;
      const adjustedDpr = largestSide > maxTextureSize ? (dpr * maxTextureSize) / largestSide : dpr;

      renderer.dpr = adjustedDpr;
    ...
    function render(time) {
      ...
      const { color: currentColor, amplitude: currentAmplitude, distance: currentDistance, enableMouseInteraction: mouseEnabled } = propsRef.current;
      program.uniforms.uColor.value.set(...currentColor);
